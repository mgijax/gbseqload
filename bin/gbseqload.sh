...
CONFIG_GBLOAD=`pwd`/gbseqload.config
echo ${CONFIG_GBLOAD}
...
if [ ! -r ${CONFIG_GBLOAD} ]
then
    echo "Cannot read configuration file: ${CONFIG_GBLOAD}" | tee -a ${LOG}
    exit 1
fi
...
cat ${CONFIG_COMMON} ${CONFIG_GBLOAD} > ${CONFIG_RUNTIME}
. ${CONFIG_RUNTIME}
...
shutDown ()
{
    ...
    if [ "${MAIL_LOG_PROC}" != "" ]
    then
        mailLog ${MAIL_LOG_PROC} "GenBanK Load - Process Summary Log" \
	    ${LOG_PROC} | tee -a ${LOG}
    fi

    if [ "${MAIL_LOG_CUR}" != "" ]
    then
        mailLog ${MAIL_LOG_CUR} "GenBank Load - Curator Summary Log" \
	    ${LOG_CUR} | tee -a ${LOG}
    fi
}
...
run ()
{
	...
	-DCONFIG=${CONFIG_GBLOAD} -DJOBKEY=${JOBKEY} ${GBSEQLOAD_APP} | \
